<section class="w3l-clients py-5" id="testimonials">
    <div class="container pt-md-5 pt-4">
        <div class="w3-testimonial-grids align-items-center py-lg-5">
            <div class="w3-testimonial-content-top">
                <div class="item mb-4">
                    <div class="testimonial-content">
                        <div class="testimonial">
                            <blockquote>
                                <q><i class="fas fa-quote-left me-2"></i>
                                In the dynamic landscape of pharmaceuticals and agrochemicals, our young and passionate team is driven to challenge the status quo. 
                                We believe in the power of collaboration, innovation, and unwavering commitment to our clients' success. 
                                Together, we're not just shaping the future of these industries; we're redefining what's possible for a healthier, more sustainable world.
                                </q>
                            </blockquote>
                            <div class="testi-des">
                                <div class="test-img"><img src="assets/images/maurayan_client.jpeg" class="img-fluid"
                                        alt="client-img">
                                </div>
                                <div class="peopl align-self">
                                    <h3>Maurayan Kharbanda</h3>
                                    <p class="indentity">Managing Director, Australia</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>